...
/**
 * Parses a list of characters representing text containing highlights.
 * @param chars List of characters to parse
 * @returns List of highlights described by the text
 */
export const findHighlights = (chars: List<number>): List<Highlight> => {
  if (chars === nil) {
    return nil;
  } else {
    const next = getNextHighlight(chars);
    if (next !== undefined) {
      const [before, highlight, after] = next;
      // const beforeHighlights = findHighlights(explode(before));
      if (before) {
        return cons({ color: 'white', text: before }, cons(highlight, findHighlights(after)));
      } else {
        return cons(highlight, findHighlights(after));
      }
      // return cons(highlight, nil);
    } else {
